 ...
 */
import type { ArrayTypedPrimitive, TypedPrimitive } from "./index.js";

/**
 * Creates a primitive that splits a comma-separated string (or maps over a pre-split
 * string array) and parses each item using the provided `primitive`. An empty or
 * missing input produces an empty array.
 *
 * @example
 * ```ts
 * const tags = array(string);      // ArrayTypedPrimitive<string>
 * const ids  = array(int.min(1));  // ArrayTypedPrimitive<number>
 * ```
 *
 * @throws {Error} When the raw value fails this primitive's validation.
 */
function array<T>(primitive: TypedPrimitive<T>): ArrayTypedPrimitive<T> {
  ...
  (fn as ArrayTypedPrimitive<T> & { _item: TypedPrimitive<T>; })._item = primitive;
  ...
  return fn as ArrayTypedPrimitive<T>;
}
