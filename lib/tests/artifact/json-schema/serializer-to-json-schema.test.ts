...
describe("toJsonSchema(token)", () => {
  it("Happy path: flat object descriptor produces { type: 'object', properties, required } with optional fields omitted from required", () => {
    const Doc = schema({
      id: uuid,
      name: str,
  ...
  it("Happy path: top-level array schema produces { type: 'array', items: <object schema> }", () => {
  ...
  it("Happy path: discriminated descriptor produces { anyOf: [...] } with the discriminant field forced as a string primitive in each branch", () => {
  ...
  it("Edge case: object with no required fields omits the required array", () => {
    const Doc = schema({
      a: optional(str),
      b: optional(int),
    });
  ...
  it("Edge case: each primitive jsonSchema is passed through verbatim", () => {
