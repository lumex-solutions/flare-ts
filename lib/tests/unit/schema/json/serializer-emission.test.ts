...
describe("schema token classification during serializer emission", () => {
  ...
  it("nested array-of-objects schema descriptors use the array helper", () => {
  ...
  it("optional nested schema and primitive fields are omitted when null", () => {
...
describe("array serialization helper construction", () => {
  it("array serialization helpers snapshot enum lookup tables at construction time", () => {
  ...
  it("required-first-field array items emit a single brace pair without double-wrapping", () => {
