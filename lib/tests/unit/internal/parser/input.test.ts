...
describe("resolveArrayInput(raw)", () => {
  ...
  it("propagates the parser message for malformed JSON", () => {
    expect(() => resolveArrayInput("[bad")).toThrow("Unexpected token 'b', \"[bad\" is not valid JSON");
  });
});
