 ...
 * typed `input` on top of {@link HandlerScope}, and handlers that run outside any contract (HTTP
...
/**
 * The DI + config base of every handler scope.
 *
 * Declared deps appear by name (lazily resolved), and `config` resolves config tokens.
 */
export type HandlerScope<D extends Record<string, ServiceToken<FlareService>> = {}> =
  & { config: ScopeConfig; }
  & InjectedMap<D>;
