/**
 * Build-time validator for lifecycle hooks used on the wrong service lifetime in the service validation pipeline.
 */
...
export class LifecycleHookValidator implements IValidator<ServiceValidationContext> {
  ...
  validate(ctx: ServiceValidationContext): ValidationError[] {
    ...
    for (const reg of ctx.controllers) {
      // ControllerClass is a bare construct-signature type with no declared prototype;
      // the runtime value is a real class, so Function exposes it.
    ...
    for (const reg of ctx.middleware) {
      // Same construct-signature-only shape as ControllerClass above.
      const proto = (reg.cls as Function).prototype;
      for (const hook of LIFECYCLE_HOOKS) {
