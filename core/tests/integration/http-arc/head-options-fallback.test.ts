// FLARE_MODE must be set before FlareHost is imported so the node adapter's
// `env: process.env` live binding sees the test-mode flag and `host.build()`
// hands back a FlareTestApp. The HEAD/OPTIONS fallback logic lives on
// HttpArc.fetch itself, but exercising it requires a fully composed app and
// `app.test()` to drive synthetic requests through the same pipeline a real
// runtime would use.
process.env["FLARE_MODE"] = "test";

import { afterAll, beforeAll, describe, expect, it } from "vitest";
import type { TestAppHandle } from "../../../src/lib/testing/test.js";
import { flareState, FlareHost, FlareResponse } from "../../../src/index.js";
import { node } from "../../../src/lib/host/runtime/node.js";

// Helpers
//
// Each scenario builds its own host so CORS policies and per-route handler
// combinations do not bleed across describe blocks. `app.test()` may only be
// called once per host instance, so a shared mutable `app` across describes
// with different routes would force `handle.reset()` calls that obscure the
// behavior under test.
...
// Primary Behavior
//
// Scope: HEAD-to-GET fallback, OPTIONS auto-Allow, CORS preflight when an
// allowed origin sends `Access-Control-Request-Method`.
// ===========================================================================
...
// Edge Cases
//
// Scope: explicit HEAD handler overrides the GET fallback; OPTIONS with Origin
// but no `Access-Control-Request-Method` falls through to auto-Allow; an
// OPTIONS preflight from a denied origin still returns 204 Allow (not 403).
// ===========================================================================
...
// Failure Modes
//
// Scope: a request whose method is not in SUPPORTED_METHODS (e.g. TRACE) on
// a matched route must return 405 with the Allow header. This is the early-405
// branch in `HttpArc.fetch`, which derives Allow from populated handler slots,
// appends HEAD when GET is present, but does NOT append OPTIONS — only `#buildAllowResponse` does.
// ===========================================================================
...
//
// Scope: HEAD-fallback runs the full pipeline against the GET descriptor
// (middleware fires, route params/contracts apply) but the body is stripped
// from the final response. CORS preflight short-circuits before the pipeline
// runs (no middleware, no handler invocation).
// ===========================================================================

// Token shared between the head-fallback pipeline's before-middleware
// (provider) and its GET handler (consumer). Declared at module scope so the
// `provides`/`state` references hand the framework the same identity. The
// value type is an object so `FlareReadonly<T>` cleanly narrows to a
// readonly property bag (string keys would force awkward FlareReadonly<string>
// indexing).
