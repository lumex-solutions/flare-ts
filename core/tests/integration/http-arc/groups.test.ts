// FLARE_MODE must be set before importing FlareHost so the default `node`
// adapter (whose `env` is bound to `process.env`) sees test mode at host
// construction time. Each test builds its own host inside its describe so a
// failed-build case does not contaminate the shared, happy-path app.
process.env["FLARE_MODE"] = "test";

import { afterAll, beforeAll, describe, expect, it } from "vitest";
import type { TestAppHandle } from "../../../src/lib/testing/test.js";
import { FlareHost, FlareResponse, MiddlewareBase, flareState } from "../../../src/index.js";
import { node } from "../../../src/lib/host/runtime/node.js";

// Shared probes & middleware fixtures
//
// All fixtures defined here are consumed by multiple describes via fresh hosts
// (one host per describe-block scope). Probes are reset inside each `it` so
// sequenced asserts are unambiguous.
...
// A handful of middleware classes named to make assertion intent obvious in
// failure output: GlobalA / GlobalB are always-on globals; GroupOnly is only
// registered inside a group's chain; ReplacementMw is the replace(from, to)
// target.
...
// A middleware that *was never registered* on the host. Used to drive the
// `exclude()` build-time validation failure.
...
// State token + middleware pair used by the cross-feature request-state case.
...
describe("Failure Modes", () => {
  it("g.exclude([X]) throws at host.build() when X was never registered as a global middleware", async () => {
    ...
    // The exclusion validation lives in build.ts and runs during
    // host.build() -> compileHttp, producing this exact message.
...
describe("Cross-Feature Interactions", () => {
  describe("with http-arc/cors", () => {
    ...
    it("g.cors(policy) overrides the arc-level CORS for routes inside the group", async () => {
      ...
      // policy is checked (origin is rejected -> no ACAO header echoed).
