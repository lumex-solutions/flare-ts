...
// hands back a FlareTestApp. Routing is compiled inside `compileHttp`, which
// runs at `host.build()`, so every routing scenario is exercised through a
// fully composed app driven via `app.test()`.
...
// Helpers
//
// Each describe block builds its own host. `app.test()` may only be called
// once per host instance and several scenarios need conflicting route sets
// (e.g. /items registered with GET+POST vs the wildcard /assets/*path app
// with no body limit), so isolating per describe keeps the assertions honest.
...
describe("Primary Behavior", () => {
  ...
  it(
    ...
    async () => {
      ...
      // Typed view: the contract's `int` primitive coerces "42" to the
...
describe("Edge Cases", () => {
  ...
  // Deferred: router literal-vs-param discrimination.
