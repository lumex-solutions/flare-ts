...
// `env: process.env` live binding sees test mode when the host is constructed.
// Every test in this file builds its own FlareHost / TestAppHandle so observed
// ordering (before/after/finally, _shapeCache deltas, registration reuse) is
// never polluted by a sibling test.
process.env["FLARE_MODE"] = "test";

import { afterEach, describe, expect, it } from "vitest";
import { model, str } from "@flare-ts/lib/schema";
import { ControllerBase, flareContract, FlareHost, FlareResponse } from "../../../src/index.js";
import {
  clearExecShapeCache,
  execShapeCacheKeys,
  execShapeCacheSize,
} from "../../../src/lib/arcs/http/exec-codegen.js";
import { Get, Post } from "../../../src/lib/arcs/http/routing/decorators.js";
import { node } from "../../../src/lib/host/runtime/node.js";

// Shared scaffolding
//
// The codegen module owns three module-level caches (_shapeCache,
// _asyncFlagsCache, _registrationCache). The first is observable via the
// exported helpers; the others are WeakMaps with no public size. Each test
// builds its own FlareHost so middleware classes, controller registrations,
// and any cache entries are scoped to the test that produced them.
//
// We re-arm FLARE_MODE in afterEach because adjacent files in the behavior
// suite mutate process.env to opt into the production runtime.
  ...
  it(
    ...
    async () => {
      ...
      class ItemsController extends ControllerBase {
        public static override deps = [];
        public static override state = [];
        public static override contract = flareContract({
          list: {},
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // Two distinct pipelines, identical shape → exactly one factory in
...
describe("Failure Modes", () => {
  ...
  // Deferred: finally-hook throws should surface via error dispatch as 500.
  it(
    ...
    async () => {
      ...
      try {
        ...
        // throwing one (caught + dispatched), then the outer one — proof
...
describe("Cross-Feature Interactions", () => {
  ...
  it(
    "(with http-arc/transport) every stage transition goes through prepareRequestBody / dispatchErrorHandlers — body validation and error dispatch both flow back to the client as a normalized Response",
    async () => {
      ...
      try {
        ...
        // Error dispatch path: handler throw → dispatchErrorHandlers →
        // user error handler → FlareResponse → normalized Response.
