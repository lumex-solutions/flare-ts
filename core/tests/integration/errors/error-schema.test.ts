...
// Build a single test app whose routes throw FlareErrors built from
// descriptors that declare an errorSchema<Shape>() detail. The HTTP arc's
// default error handler (handleControllerError) serialises the FlareError
// into a JSON body of { error, code?, detail? }; that lets the test prove
// the detail typed as Shape survives end-to-end into the response when
// expose=true, and is omitted when expose=false.
...
describe("Failure Modes", () => {
  it("attempting to mutate the frozen errorSchema object is rejected or ignored, and the brand survives across reuse", () => {
    ...
    // accept either branch (strict throw or silent non-strict drop) per spec
    // language, but the brand must remain intact after the attempt.
