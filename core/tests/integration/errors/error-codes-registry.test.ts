...
} from "../../../src/lib/errors/types/types.js";
import { FlareHost } from "../../../src/lib/host/flare-host.js";
import { node } from "../../../src/lib/host/runtime/node.js";

// Module-scope registry built via `flareErrorCodes`. Because the call is at
// module top-level, it stands in for the spec's "registry built at module
// load" — any throw in `flareErrorCodes` here would abort the import of this
// file, matching the contract that misconfigurations surface eagerly. The
// happy-path registry below proves the inverse: a well-formed descriptor
// produces a token that is consumable downstream.

type ResourceDetail = { readonly resource: string; readonly id: string; };

// Typed view of the registry the framework produces at runtime. The framework's
// public type for `flareErrorCodes`'s return narrows entries to `unknown` (the
// constraint uses `Record<string, unknown>`, which erases per-entry shape), so
// the test casts to this view to recover the entry-level guarantees the
// downstream code depends on (e.g. `CodeDescriptor<ErrorSchema<ResourceDetail>>`
// driving `new FlareError(...)`'s detail-argument requirement at the type level).
...
describe("Edge Cases", () => {
  it("freezes the built registry so runtime mutation of an entry's expose flag does not affect subsequent FlareError construction", () => {
    ...
    // throws TypeError. Accept either strict-throw or silent-drop per the
    // spec's wording, and assert the value is unchanged after the attempt.
...
describe("Cross-Feature Interactions", () => {
  ...
  it(
    ...
    async () => {
      ...
      const sideHost = new FlareHost(node);
      for (const category of categories) {
