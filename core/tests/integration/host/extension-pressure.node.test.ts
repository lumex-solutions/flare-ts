// End-to-end tests for a realistic host extension: an API-key auth extension that registers config, a
// scoped verifier service, and global auth middleware, and returns a state token the route reads. The
// tests drive real requests through the test app to exercise the auth flow.
...
import { FlareHost, FlareResponse, FlareService, flareConfig, flareState } from "../../../src/index.js";
...
// --- The realistic extension under test: API-key auth ---------------------------------------------
...
// --- Adapter (test-mode node, in-memory config, silent logs) --------------------------------------

class SilentTransport extends LoggerTransport {
  static override readonly transportName = "silent-pressure";
  static override deps = [];
...
describe("host extension API: realistic auth extension end-to-end", () => {
  it("config + scoped service + global middleware + a returned state-token member compose through real requests", async () => {
    const host = new FlareHost(authAdapter(cfg()), [apiKeyAuthExt]);
  ...
  it("the installer composes exactly once, at construction", () => {
    ...
    installCount = 0;
    new FlareHost(authAdapter(cfg()), [apiKeyAuthExt]);
