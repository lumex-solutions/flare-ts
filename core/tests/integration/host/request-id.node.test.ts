// FLARE_MODE is set process-wide so any incidentally-constructed host that
// reads `process.env` would land in test mode. Tests below opt OUT by
// constructing custom adapters with an empty `env` so the FlareHost
// constructor doesn't see the flag and `host.build()` returns the live
// FlareAppNode (not the test-mode shim that bypasses runtime header stamping).
