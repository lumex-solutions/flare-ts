...
// Adapter helpers — inject test config and either drop the runtime defaults
// (so a test observes only its user-registered transports) or substitute a
// recording stand-in for the default (so a test can assert relative position
// of the runtime default versus user transports).

function makeNodeAdapter(config: JsonObject) {
  return nodeAdapter(config, { FLARE_MODE: "test" }, { defaultLoggerTransports: [] });
}

// A recording stand-in for the runtime default transport. The spec talks
// about "runtime defaults" abstractly and asserts only relative order, so we
// substitute a recording transport class into `defaultLoggerTransports` and
// observe its position in the live transport list (and in fan-out order).
...
// Recording fixtures. `fanout` captures every write across all transports in
// the order it occurred, so a single log call can be inspected for relative
// transport-order.
...
describe("Primary Behavior", () => {
  ...
  it("fires the runtime default first, then user transports A and B in registration order, for each log record", async () => {
    ...
    try {
      ...
      // For the single log call, the fan-out order is: default → A → B.
...
describe("Failure Modes", () => {
  ...
  it("throws at logger bootstrap when a registered transport class's constructor cannot accept (container)", () => {
    ...
    // field can be touched — that is the bootstrap-time failure the spec
    // describes.
    //
    ...
    // a ReferenceError mentioning super — that's the bootstrap-time failure.
...
describe("Cross-Feature Interactions", () => {
  ...
  it("transport list is frozen at bootstrap; further host.logging.transport(...) calls after build have no effect on the live logger (with host/logger-bootstrap)", async () => {
    ...
    try {
      ...
      // logger — the transport list is frozen at bootstrap.
