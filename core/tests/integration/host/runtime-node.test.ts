// FLARE_MODE is set process-wide so any incidental `node`/`cf` adapter would
// land in test mode and short-circuit `host.build()` to a TestAppHandle shim.
// Every test here is about the LIVE FlareAppNode http.Server path, so each
// test builds its own adapter with an empty `env` to keep FlareHost out of
// test mode and `node.createApp(host)` returning a real FlareAppNode.
...
// Silent transport so the live FlareAppNode can boot without spamming stdout
// during the test run. Tests that need to observe log records add their own
// RecordingTransport instead of (or alongside) this one.
...
/** Recording transport — every record is appended; per-test reset keeps assertions scoped. */
  ...
  it("handler throw after headers sent destroys the connection and logs a warning instead of double-writing", async () => {
    ...
    try {
      ...
      try {
        ...
        // Network-level abort during streaming — also valid.
        errored = true;
      }
...
describe("Cross-Feature Interactions", () => {
  ...
  it("(with host/request-id) When host.requestIdHeader === true, the response includes x-request-id (real handler path + 500 error path)", async () => {
    ...
    // Both must emit the header. (The 503 shutdown path is verified
    // separately — deferred here because triggering a request while
    // #isShuttingDown is true without racing the shutdown is brittle.)
    const host = new FlareHost(nodeAdapter({
    ...
    try {
      ...
      // Format is `<8-hex-nonce>-<sequence>` — see request-id feature spec.
      ...
      // Sequences differ — proves each branch grabbed its own id from the
  ...
  // The following Cross-Feature bullets are intentionally NOT implemented
  // Omitted here — requires a real Node listen socket and signal handlers:
  //
  // - "(with host/graceful-shutdown) SIGTERM triggers drain; in-flight
  //   requests complete then process exits 0"
  //     Triggering SIGTERM inside the test process would call
  //     `process.exit(0)` via FlareAppNode.#shutdown's `exitProcess: true`
  //     branch, terminating the vitest worker before subsequent tests run.
  //     Verifiable only with a child-process harness.
  //
  // - "x-request-id 503 shutdown path"
  //     Reaching the 503 branch requires a request to land while
  //     `#isShuttingDown === true`. handle.stop() resolves only after the
  //     server has closed connections, so issuing a request mid-shutdown
  //     races the close and is flaky in a portable test. Verifiable with
  //     a controlled fixture that gates shutdown progression.
});
