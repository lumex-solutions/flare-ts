// FLARE_MODE must be set before any host adapter sees `process.env` so that
// `host.build().test()` is allowed. The node adapter reads `process.env` live,
// but several scenarios below also override `env` on a custom adapter.
