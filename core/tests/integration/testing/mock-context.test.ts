...
// Shared state tokens used across the unit + integration arms.
// `TenantState` has a default to exercise default-resolution under mockContext.
// `AuthState` has no default to assert require()-throws when unseeded.
// `EchoState` is what the middleware writes for the "headers in / state out" test.
...
// A handler that reads x-tenant header, optionally requires AuthState, and
// emits the headers + state-derived payload as JSON. Used by both the unit
// arm (invoked via mockContext) and integration arm (via app.fetch).
...
// before() middleware that reads a header and writes a state token.
// Exercised both inline (in the unit arm) and through the host pipeline.
