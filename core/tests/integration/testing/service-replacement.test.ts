...
// `env: process.env` live binding picks it up during host construction. The
...
// Shared fixtures
//
// Each describe block builds its own FlareHost so the test-mode `#handleIssued`
// flag, `#originalSingletonRegs` snapshot, and singleton instances do not leak
// between cases. The feature under test is `AppTestOptions.replace`, so every
// test exercises that argument on either `host.build().test({ replace })` or
// `handle.reset({ replace })`.
