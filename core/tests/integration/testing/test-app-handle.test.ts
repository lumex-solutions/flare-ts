...
import { Method } from "../../../src/lib/arcs/http/routing/decorators.js";
...
// Shared probes & helpers
//
// Several tests inspect what the handler observed (raw body, headers, signal
// state) and what the harness produced (response object, headers, cookies).
// We use module-scoped probes so an inline handler can record observations
// without us needing to thread state through closure-captured controllers.
...
describe("Failure Modes", () => {
  ...
  beforeAll(async () => {
    ...
    // A handler that throws if its body is not a non-empty string. We don't
    // use a full contract here -- the spec bullet is about the handler's own
    // 400 path not being intercepted by the harness, which any handler-emitted
    // 400 satisfies. Throw via FlareResponse(400, ...).
