/**
 * In-process integration tests for FlareTestApp under FLARE_MODE=test: build()
 * returns the test variant, test() issues a working TestAppHandle, run/export
 * are null shims, and replace maps validate at compile time. FLARE_MODE must
 * be set before imports so the node adapter's env binding sees it during host construction.
 */
...
/** Builds an isolated host with a ping route; optional Probe singleton records lifecycle calls. */
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // replacement class. A second compile throws "may only be called
        // once" from [COMPILE_FOR_TEST]; reaching this point proves a single
        // compile pass.
...
describe("Failure Modes", () => {
  ...
  it(
    ...
    async () => {
      ...
      process.env["FLARE_MODE"] = "test";
      const failingHost = new FlareHost(nodeAdapter({}));
...
describe("Cross-Feature Interactions", () => {
  ...
  it(
    ...
    async () => {
      ...
      // been constructed; validation lives at compile time, not
      ...
      process.env["FLARE_MODE"] = "test";
      const host3 = new FlareHost(nodeAdapter({}));
  ...
  it(
    "(with host) The host transitions through starting to ready during a "
      + "normal app.test() and draining to ready during handle.reset()",
    async () => {
      // HostState is `starting | ready | draining | stopped`; no `compiling`
      // value exists. Test the observable transitions: starting before
      ...
      const handle = await host.build().test();
      try {
        // starting to ready: FlareTestApp.test() calls SET_HOST_STATE("ready")
        // immediately after startAsync resolves.
        expect(host.state).toBe("ready");

        // draining to ready: handle.reset() walks the host through
