/**
 * In-process integration tests for the service-validation pass inside host.build():
 * dependency graph, captive deps, lifecycle hooks, and registration checks.
 * Replace-path tests require FLARE_MODE=test before any FlareHost is constructed
 * so the node adapter's env binding sees the flag during host construction.
 */
...
describe("Primary Behavior", () => {
  ...
  it(
    ...
    () => {
      ...
      // CAPTIVE_DEPENDENCY for a singleton depending on a scoped service.
      ...
      // Captive deps surface at build() time. With FLARE_MODE=test the service
      // validator runs inside `host.build()` before the test-mode singleton-compile
      // is deferred, so FlareValidationError fires (not FlareTestError).
  ...
  it(
    "a direct service cycle (A depends on B, B depends on A) fails build with CIRCULAR_DEPENDENCY showing the A-B-A cycle path",
    () => {
      ...
      // The reported cycle path uses `<start>`, `<next>`, and `<start>` again.
...
describe("Edge Cases", () => {
  ...
  it(
    "when the service dependency graph contains any UNDECLARED_DEPENDENCY, cycle detection is skipped and the build error lists the undeclared entries only",
    () => {
      ...
      // Cycle between Cyc1 and Cyc2 via post-declaration assignment so both classes
...
describe("Failure Modes", () => {
  ...
  it(
    ...
    () => {
      // Two disjoint cycles between A and B and between C and D. Each is reported exactly
  ...
  it(
    "controllers and middleware are sourced from groups too; a controller registered in a group still gets checked",
    () => {
...
describe("Cross-Feature Interactions", () => {
  it(
    ...
    async () => {
      ...
      const goodHost = new FlareHost(nodeAdapter({}));
      class A extends FlareService {
        public static override deps = [];
      }
      class B extends FlareService {
  ...
  it(
    ...
    async () => {
      ...
      class LogCtrl extends ControllerBase {
        public static override deps = [Logger];
        public static override state = [];
        // Route /ok uses health() so the handler name does not collide with
        // ControllerBase's protected ok() response-helper method.
        @Get("/ok")
      ...
      const app = await host.build().test();
      try {
        // build() succeeding is the assertion: no UNDECLARED_DEPENDENCY against
        // Logger from LogService, no CONTROLLER_UNREGISTERED_DEP against Logger
        // from LogCtrl.
  ...
  it(
    "(with validation/composite) Run order is DependencyValidator, then CaptiveDependencyValidator, then LifecycleHookValidator, then ServiceRegistrationValidator; ordering is observable in the error list",
    () => {
      ...
      // Ordered: dependency, then captive, then lifecycle, then registration.
  ...
  it(
    ...
    () => {
      // Two distinct, independent service-pass errors. The build throws once,
      // not once per error, and the carried `errors` array holds every entry.
