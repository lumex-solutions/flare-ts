/**
 * In-process integration tests for the service container: scoped and singleton
 * resolution, config tokens, error propagation, and cross-feature wiring with
 * HTTP routes and host replace.
...
/** Returns a unique per-instance label stamped onto fixture services. */
...
/** Resets rendezvous primitives used by the concurrent singleton race route. */
...
/** Holds an injected SharedScoped so a controller can resolve the same token through two paths. */
...
/** Token never registered on the host; used only to trigger the framework unregistered error. */
class UnregisteredService extends FlareService {
  public static override deps = [];
}

/** Resolves an unregistered token in its constructor to exercise the runtime not-registered branch. */
...
/** Self-referential cycle invisible to build-time validation; resolved via container.resolveDep at runtime. */
...
/** Captures a per-request mark set by middleware so the handler can prove container identity. */
...
/** Builds the shared host wiring every container fixture route except the replace test. */
function buildHost() {
  process.env["FLARE_MODE"] = "test";
...
describe("Primary Behavior", () => {
  ...
  it("resolves a config token via this.config(token) using the value computed at host.build() time", async () => {
    ...
    // The `int` descriptor parses the env-var string "42" into number 42 at
    // host.build() time; resolveCfg returns the already-parsed value.
