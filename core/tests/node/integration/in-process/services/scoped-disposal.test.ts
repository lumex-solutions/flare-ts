/**
 * In-process integration tests for scoped service dispose(): LIFO ordering,
 * async awaiting, failure isolation, and integration with singletons, host
 * stop, and the HTTP arc. FLARE_MODE must be set before imports so the node
...
/** Composes the shared host for most suites; stop and middleware-error cases use isolated hosts. */
...
describe("Primary Behavior", () => {
  it("invokes dispose() on three scoped services exactly once each in reverse creation order", async () => {
    ...
    // walk reverse: RootA, then MidA, then LeafA. Each service's dispose() pushes
