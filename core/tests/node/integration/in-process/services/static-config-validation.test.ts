/**
 * In-process integration tests for static config declaration validation on
 * FlareBase.config(). Exercises guardrails before Container.resolveCfg under
...
/** Empty-descriptor token with no flare.json section; exercises missing-key resolveCfg behavior. */
...
/** FlareBase subclass (not FlareService) used to prove config validation lives on FlareBase. */
...
class GhostReaderController extends ControllerBase {
  ...
  public async show() {
    ...
    // The schema parses {} to {}, so the section keys are absent. Asking for
...
function buildSharedHost() {
  process.env["FLARE_MODE"] = "test";
  const host = new FlareHost(nodeAdapter(SHARED_FLARE_JSON));
