/**
 * Pins logger error(err, msg) record shape: normalized error field, default messages,
 * overload disambiguation, and pretty-mode ConsoleTransport rendering.
...
/** Captures every log record so error-field shape can be asserted on the transport. */
...
/** Strips ANSI escapes so pretty-mode renderings can be matched against visible content. */
...
describe("Cross-Feature Interactions", () => {
  ...
  it("toErrorField is the same normalizer used by the host's last-resort error reporting (shape consistency)", async () => {
    ...
    // logger attaches to a record for the same input.
