/**
 * In-process integration tests for FlareHost.state lifecycle transitions, readiness
 * routing, middleware visibility during shutdown, and build-failure state retention.
...
/** Builds a fresh host with readiness and state routes; optional middleware records host.state observations per request. */
function buildHost(opts: { observations?: StateObservation[]; } = {}) {
  ...
  if (opts.observations) {
    ...
    // Anonymous middleware class captures host.state from inside the request
    // lifecycle.
...
describe("Cross-Feature Interactions", () => {
  it(
    ...
    async () => {
      ...
      // after startAsync resolves, using the same SET_HOST_STATE the Node and
      // CF runtimes use, driven by the test-mode runtime).
      const app = await host.build().test();
      try {
        expect(host.state).toBe("ready");
