/**
 * Pins request-scoped flareState resolution: middleware provides, controller
 * requires, defaults, derivations, caching within a request, and logging mappers.
...
/** Token with a default: resolve falls through to `getTokenDefault` when nothing sets it. */
const DefaultedState = flareState<{ region: string; }>("DefaultedState").withDefault({ region: "us-east" });

/** Token with a derivation that runs once per request and caches the result. */
...
/** Token whose derivation returns undefined so resolve falls back to the default. */
...
/** Isolated-route token resolved via `.withDefault()` without appearing in route `state`. */
const IsolatedDefaultState = flareState<{ tag: string; }>("IsolatedDefaultState").withDefault({
  tag: "isolated-default",
});

/** Token whose log mapper merges fields onto the ALS logger store on set(). */
...
describe("Cross-Feature Interactions", () => {
  it("(with http-arc/pipeline-codegen) state written in before is visible to after and finally hooks", async () => {
    ...
    try {
      ...
      // after, and finally on the same FlareHttpContext, so both downstream hooks
