/**
 * Exercises graceful shutdown on the production FlareAppNode over a loopback socket.
 * process.exit is captured so vitest stays alive; signal listeners are snapshotted and
 * restored between tests. FLARE_MODE is intentionally unset so builds use FlareAppNode.
 */
...
import type { FlareAppBase } from "../../../../../src/lib/host/flare-app-base.js";
...
/**
 * Picks out the last listener installed for an event; by construction this
 ...
 * every other listener (including vitest's own) and only the framework's
 * handler runs.
 */
...
describe("signal-triggered shutdown", () => {
  ...
  it("SIGINT mirrors SIGTERM (state advances to stopped, exit code 0)", async () => {
    const ctx = await startApp();
...
describe("handle.stop() shutdown without process exit", () => {
  it(
...
describe("shutdown drain timeout and force-close", () => {
  it(
    ...
    async () => {
      ...
      // The handler signals when it has actually been ENTERED, so stop() below provably races an
      // in-flight request. Without this signal, stop() could run before the request reaches the
      // handler, in which case the drain would complete instantly instead of timing out.
      ...
      // Fire the hanging request and DO NOT await it (it never completes), then wait until the
      // handler has been entered - the request is definitely counted as active from here.
...
describe("idempotent shutdown invocation", () => {
  it(
...
describe("force-exit timer ref handling", () => {
  it(
...
describe("request rejection during drain", () => {
  it(
    ...
    async () => {
      ...
      // The handler signals when it has actually been ENTERED, so stop() provably races an in-flight
      // request (a fixed sleep is flaky under full-suite CPU load).
      let enteredResolve!: () => void;
      const entered = new Promise<void>((r) => (enteredResolve = r));
      ...
      // Start the slow request; do not await. Wait until the handler has been entered - the request
      // is definitely counted as active from here.
...
describe("process error handler shutdown", () => {
  it(
...
describe("stopAsync failure during shutdown", () => {
  it(
...
describe("shutdown timeout with exitProcess false", () => {
  it(
...
describe("host state lifecycle during shutdown", () => {
  it(
    ...
    async () => {
      ...
      class Probe extends FlareService {
        public static override deps = [];
        public override async onStop(): Promise<void> {
          observed.push(host.state); // captured mid-shutdown while state is "draining"
        }
      }
...
describe("singleton stopAsync ordering during shutdown", () => {
  it(
...
describe("server listening through drain", () => {
  it(
    ...
    async () => {
      ...
      // Entered-signal instead of a fixed sleep: stop() must provably race an in-flight request.
      let enteredResolve!: () => void;
      const entered = new Promise<void>((r) => (enteredResolve = r));
      ...
      // In-flight request; do not await. Wait until the handler has been entered.
      const slow = httpGet(ctx.port, "/slow");
      await entered;
