/**
 * Integration tests for FlareHost test mode: build(), test({ replace }), reset(),
 * and related failure paths. FLARE_MODE must be set before importing FlareHost
 * so the node adapter's env binding sees it during host construction.
 */
...
/** Builds a node adapter with a caller-supplied env map for the no-FLARE_MODE failure path. */
...
/** Builds a fresh host with one singleton OriginalService reachable through GET /value. */
...
describe("Primary Behavior", () => {
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // Singletons map agrees with the route output, proving
  ...
  it("pre-built singletons such as Logger remain in the singleton map and are reused when the test handle recompiles with new replacements", async () => {
    const host = buildHostWithSingleton();
    const handle = await host.build().test({
      replace: new Map([[OriginalService, StubService]]),
    });
...
describe("Edge Cases", () => {
  it(
    ...
    async () => {
      ...
      //   2. Both instances are ScopedStub, not ScopedOriginal, confirming
  ...
  it(
    "Two replacements in one call apply atomically without leaving the host in a half-mutated state",
    async () => {
      ...
      // Second call with a valid map must observe OriginalService
...
describe("Failure Modes", () => {
  it("app.test() without FLARE_MODE=test throws FlareTestError with explicit env-set guidance", async () => {
    ...
    // app, NOT FlareTestApp. COMPILE_FOR_TEST carries the explicit guidance message,
