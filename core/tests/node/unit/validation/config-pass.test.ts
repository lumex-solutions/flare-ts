/**
 * Integration tests for config-pass validation: registered tokens, missing keys and
 * fields, and composite validator ordering. FLARE_MODE must be set before any
 * FlareHost is constructed so the node adapter's env binding sees it during host construction.
 */
...
/** Wraps the node adapter with a synthetic flare.json for hosts that lack a repo file. */
...
describe("Primary Behavior", () => {
  ...
  it("a host where a registered token's top-level key is absent from flare.json -> build throws with MISSING_CONFIG_KEY", () => {
    ...
    // matching the shape the validator receives when a section is absent.
