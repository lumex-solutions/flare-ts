...
describe("Container (constructor)", () => {
  it("constructs with all defaults — empty registry, no singletons, empty config", () => {
    ...
    // No registrations -> resolveDep should throw with the not-registered message.
...
describe("Container.resolveCfg", () => {
  ...
  it("returns undefined cast as T when key is missing (current behavior — no throw)", () => {
...
describe("Container.resolveDep", () => {
  ...
  it("resolves a scoped service via registry.get(token).factory(this) and caches it in #instances", () => {
    class Svc extends FlareService {}
    ...
    expect(first).toBeInstanceOf(Svc);
  });
  ...
  it("returns the transitive dep correctly when a factory calls back into container.resolveDep(otherToken)", () => {
    ...
    expect(root.injected).toBeInstanceOf(Dep);
  ...
  it("cleans up #resolving in finally when a factory throws, so a later resolve does not report a false circular error", () => {
    class Svc extends FlareService {}
    ...
    shouldThrow = false;
    const second = container.resolveDep(Svc);
    expect(second).toBeInstanceOf(Svc);
    expect(factory).toHaveBeenCalledTimes(2);
  });
});

describe("Container.dispose", () => {
  it("returns undefined synchronously when #instances is empty (no Promise allocated)", () => {
    const container = new Container();
  ...
  it("catches a sync dispose() that throws and still runs subsequent disposes", () => {
    ...
    const registry = new FlareRegistrationMap();
    // Insertion order: Other, Throws. Reverse -> Throws first, then Other.
  ...
  it("catches an async dispose() rejection and still runs subsequent disposes", async () => {
    ...
    const registry = new FlareRegistrationMap();
    // Insertion: Other, Rejects -> reverse: Rejects first (async), then Other (chained).
    registry.set(Other, makeRegistration(Other, (c) => new Other(c)));
  ...
  it("does not prevent later async disposes from completing when an earlier sync dispose throws", async () => {
    ...
    const registry = new FlareRegistrationMap();
    // Insertion: AsyncTail, SyncHead -> reverse: SyncHead (throws), AsyncTail (async).
