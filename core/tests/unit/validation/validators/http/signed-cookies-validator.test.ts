...
describe("SignedCookiesValidator.validate", () => {
  it("returns [] when a secret is configured, even if routes declare signedCookies", () => {
    class Ctrl {
      static contract = { [CONTRACT_BRAND]: true, login: { signedCookies: true } } as never;
    }
  ...
  it("returns [] when no route declares signedCookies and no secret is configured", () => {
    class Ctrl {
      static contract = { [CONTRACT_BRAND]: true, getUser: { route: {} } } as never;
    }
  ...
  it("emits one error per declaring entry across controllers", () => {
    class A {
      static contract = { [CONTRACT_BRAND]: true, a: { signedCookies: true }, b: { signedCookies: true } } as never;
    }
    class B {
      static contract = { [CONTRACT_BRAND]: true, c: { signedCookies: true }, d: {} } as never;
    }
