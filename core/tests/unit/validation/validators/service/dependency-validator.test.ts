...
/**
 * Build a minimal FlareServiceClass with the supplied deps. DependencyValidator
 * reads `cls.deps` and `token.name` only; it never instantiates the class.
 */
...
describe("DependencyValidator.validate", () => {
  ...
  it("skips cycle detection when any UNDECLARED_DEPENDENCY is reported (returns only the undeclared errors)", () => {
    // Build a graph that has BOTH an undeclared dep and a cycle. If cycle
