...
// Minimal stand-in for the `target` argument that decorators receive. The
// decorator only reads `target.constructor.name` (for error formatting), so an
// inline arrow function whose constructor name is "Function" suffices for
// happy-path assertions, while a wrapper function lets us assert error message
// content for error-condition cases.
...
// Build a fresh per-test ClassMethodDecoratorContext-shaped object. The
// decorators only read `context.metadata`; everything else can be omitted.
...
describe("Get / Post / Put / Patch / Delete / Head / Options (all forward to Method)", () => {
