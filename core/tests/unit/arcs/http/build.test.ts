...
import type { ContractToken } from "../../../../src/lib/arcs/http/composition/contract/flare-contract.js";
...
import { flareContract } from "../../../../src/lib/arcs/http/composition/contract/flare-contract.js";
import { clearExecShapeCache } from "../../../../src/lib/arcs/http/exec-codegen.js";
import { joinRoutePath } from "../../../../src/lib/arcs/http/routing/path.js";
...
/**
 * Tests for `core/src/lib/arcs/http/build.ts`. The unit boundary is the file:
 * we construct realistic ControllerRegistration / MiddlewareRegistration values
 * by hand and feed them through `compileHttp`, then assert against the returned
 * pipelines / middleware / router / execFns. Internal helpers
 * (`compilePipelines`, `compileExecStepNames`, `getMiddlewareIdxs`,
 * `processMwRegistration`, `verifyProvidedState`, `compileRoutes`,
 * `compileQueryPrimitives`, `compileResponseSerializers`, `scoreRoute`) are
 * exercised transitively through `compileHttp`.
 */
...
describe("compilePipelines (via compileHttp)", () => {
...
describe("compileExecStepNames (via execNames passed to compileExecFn — observable via exec)", () => {
...
describe("compileMiddleware (via compileHttp.middleware)", () => {
...
describe("getMiddlewareIdxs (via compileHttp)", () => {
...
describe("verifyProvidedState (via compileHttp)", () => {
...
describe("compileRoutes (via compileHttp)", () => {
...
describe("compileQueryPrimitives (via compileHttp)", () => {
...
describe("compileResponseSerializers (via compileHttp)", () => {
  ...
  it("compiles serializers indexed by methodIdx then by numeric status for multiple response statuses on one method", () => {
    ...
    // ResponseSerializers is now a sparse Array<Record<number, Serializer>>
    // indexed by methodIdx (0 = GET) → { [status]: serializer }.
...
describe("scoreRoute (via flareRoute.score)", () => {
