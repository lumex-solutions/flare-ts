...
import { INVALID_REQUEST_PATH_BODY } from "../../../../src/lib/arcs/http/routing/request-path.js";
...
/**
 * Tests for `core/src/lib/arcs/http/http-arc.ts`. Lifecycle symbols, `group()`,
 * and the `fetch()` path are unit-testable by constructing a minimally fake
 * `IFlareHost` and invoking the arc directly. Deeper paths inside `#executePipeline`,
 * `#extractRouteParams`, `#extractQueryParams`, `#parseQuery`, `#assertSingleValue`,
 * and `#findQueryValue` are private and only reachable through `fetch()` after a
 * full `[COMPILE_HTTP_ARC]` has run with realistic registrations; deeper pipeline
 * branches are covered by integration tests under `core/tests/integration/http-arc/`.
 */
