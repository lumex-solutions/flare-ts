...
describe("SET_HOST_STATE / COMPILE_FOR_TEST / RESET_FOR_TEST", () => {
  it('each export is a unique symbol (`typeof === "symbol"`)', () => {
    expect(typeof SET_HOST_STATE).toBe("symbol");
    expect(typeof COMPILE_FOR_TEST).toBe("symbol");
    expect(typeof RESET_FOR_TEST).toBe("symbol");
  });
...
describe("UNSAFE_CONFIG_ENV_KEYS", () => {
  it("is a `Set<string>` containing exactly `__proto__`, `prototype`, `constructor`", () => {
    expect(UNSAFE_CONFIG_ENV_KEYS).toBeInstanceOf(Set);
