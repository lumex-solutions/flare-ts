...
// Test fixtures — FlareAppBase reads `host.http`, `host.singletonServices`, and
// `host.logger`. We build a stub host with controllable behaviour plus a concrete
// FlareAppBase subclass since the base is abstract.
...
/** Minimal FlareService stub. We do not extend FlareService directly because the
 *  Logger detection compares against the real Logger class — these stand-ins
 *  must NOT be instanceof Logger. */
class FakeService {
  static order: string[] = [];
  static reset(): void {
    FakeService.order = [];
...
describe("FlareAppBase.test (base implementation)", () => {
...
describe("FlareAppBase.start", () => {
  ...
  it("Logger instance is started first (detected via instanceof Logger) and #loggerStarted flips to true", () => {
  ...
  it("#singletonIdx advances by one per singleton walked: stop() walks the same count in reverse", () => {
...
describe("FlareAppBase.startAsync", () => {
  ...
  it("Logger started first via instanceof check, #loggerStarted flips true; non-Logger services emit start/ready trace events", async () => {
...
describe("FlareAppBase.stop", () => {
  it("walks singletons in reverse insertion order skipping Logger, invokes http[STOP_HTTP_ARC], then stops Logger last when #loggerStarted", () => {
    const logger = new StubLogger();
    class A {
      onStart(): void {}
      onStop(): void {
  ...
  it("a service onStop returning a Promise during sync runtime lifecycle throws via #assertSync", () => {
    class BadSvc {
      onStart(): void {}
      onStop(): Promise<void> {
        return Promise.resolve();
  ...
  it("when #loggerStarted is false, Logger.onStop is not called even if a Logger is registered", () => {
...
describe("FlareAppBase.stopAsync", () => {
  it("walks singletons in reverse order skipping Logger, awaits arc stop, then awaits Logger.onStop only when #loggerStarted", async () => {
    const logger = new StubLogger();
    class A {
      async onStart(): Promise<void> {}
      async onStop(): Promise<void> {
...
describe("FlareAppBase (private) #assertSync (exercised transitively)", () => {
  ...
  it("Promise result throws '[flare] <label> returned a Promise during sync runtime lifecycle.'", () => {
    // Already covered for onStart by FlareAppBase.start tests; this also asserts
    // the exact label substitution for the Logger.onStart() call path.
    const logger = new StubLogger();
