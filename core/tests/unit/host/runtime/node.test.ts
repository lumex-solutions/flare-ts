...
describe("node adapter (module-scope constant)", () => {
  ...
  it("createApp(host) returns a FlareAppNode", () => {
    // Minimal stub satisfying the FlareAppBase constructor's `host.http` read.
    const host = { http: {} } as never;
    const app = node.createApp(host);
    expect(app).toBeInstanceOf(FlareAppNode);
  });

  it("createLogger(transports, container) returns a Logger", () => {
    const container = new Container(
      new FlareRegistrationMap(),
      new Map(),
      { log: { level: "info" } } as unknown as JsonObject,
    );
    const logger = node.createLogger([], container);
    expect(logger).toBeInstanceOf(Logger);
  });

  it("createTestRequest(input) delegates to buildNodeTestRequest — returns a FlareRequest with matching method/url", () => {
