...
describe("FlareHost.build", () => {
  ...
  it("runs #compileConfig then #compileLogger before #build (logger is available after build)", () => {
    const host = new FlareHost(makeAdapter());
    registerMinimalPingRoute(host);
    host.build();
  ...
  it("in #testMode, scoped/singleton compile is deferred to [COMPILE_FOR_TEST]", () => {
