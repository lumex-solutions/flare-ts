...
/** A second transport class so per-transport overrides can be exercised with a distinct name. */
...
/** Transport whose `transportName` static is missing — to test the undefined-key fallback. */
...
describe("Logger", () => {
  ...
  it("flushBootstrapBuffer drains records previously written by _log() in registration order", async () => {
...
describe("toErrorField (exported)", () => {
...
describe("_log (exported)", () => {
  ...
  it("buffered records carry timestamp (number), level, message, and meta (when provided), but no context", async () => {
    _log("info", "x", { a: 1 });
