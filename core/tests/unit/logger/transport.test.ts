...
describe("LoggerTransport (abstract)", () => {
  it("cannot be instantiated directly: concrete subclass with a stub write is the test vehicle", () => {
    ...
    expect(t).toBeInstanceOf(LoggerTransport);
  ...
  it("static deps defaults to an empty array on the base; concrete subclasses can override without runtime error", () => {
    ...
    const t = new WithDeps(makeContainer());
    expect(t).toBeInstanceOf(LoggerTransport);
  });
...
describe("CFWLoggerTransport (abstract)", () => {
  it("a concrete subclass is also instanceof LoggerTransport", () => {
    class CFWStub extends CFWLoggerTransport {
      static override readonly transportName = "cfw-stub";
      static override deps: never[] = [];
      override write(): void {
        // no-op
      }
    ...
    const t = new CFWStub(makeContainer());
    expect(t).toBeInstanceOf(CFWLoggerTransport);
    expect(t).toBeInstanceOf(LoggerTransport);
  });
