/** Unit tests for FlareRequest body buffering, signals, and route param setters. */
...
/** Builds a minimal RequestAdapter with overridable rawHeaders and signal. */
...
/** Produces an async iterable from byte chunks for iterable-native request bodies. */
...
describe("initial request fields and cached path", () => {
...
describe("buffer", () => {
  ...
  it("Aborts when signal already aborted before reading", async () => {
    ...
    // Deliberately primes #signal before buffer() so the abort path sees a populated field.
  ...
  it("Aborts when signal aborts mid-stream", async () => {
    ...
    const req = new FlareRequest(adapter, "POST", "/", "r", iter);
    // Deliberately primes #signal before buffer() so mid-stream abort is observed.
    void req.signal;
...
describe("text / json / stream", () => {
  ...
  it("stream: enforces max body bytes while iterating", async () => {
    const req = new FlareRequest(
      ...
      "/",
      "r",
      asyncIterableOf([new Uint8Array(4), new Uint8Array(4)]),
    );
