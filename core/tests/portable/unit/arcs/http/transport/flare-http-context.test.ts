/** Unit tests for FlareHttpContext extract, setters, serializers, and cookie handling. */
import { describe, it, expect } from "vitest";
import type { RequestDescriptor } from "../../../../../../src/lib/arcs/http/composition/contract/http-contract.js";
...
describe("request-scoped state storage and resolution", () => {
...
describe("lazy cookie jar initialization", () => {
...
describe("contract-based request context extraction", () => {
...
describe("response serializer lookup", () => {
  it("Returns the per-(methodIdx, status) serializer when present; undefined otherwise", () => {
    const ctx = new FlareHttpContext(makeReq());
    // ResponseSerializers are indexed by methodIdx to per-status serializer maps.
    ...
    // Missing methodIdx slots return undefined.
...
describe("[SET_REQ_CTX]", () => {
  ...
  it("Sets responseSerializers when provided", () => {
    ...
    // Re-issue with undefined preserves installed serializers.
    ctx[SET_REQ_CTX]();
    expect(ctx.serializer(0, 201)).toBe(ser201);
  });
});
...
describe("cookie header parsing and lookup", () => {
  ...
  it("Parsed result cached on second call", () => {
    ...
    // Same reference indicates a cache hit.
    expect(second).toBe(first);
  });
});

describe("Set-Cookie emission", () => {
...
describe("cookie deletion via expiring Set-Cookie", () => {
