/**
 * Unit tests for {@link CompositeValidator} ordering, aggregation, and short-circuit behavior.
 */
...
describe("validator composition and aggregation", () => {
  ...
  it("a second call to validate(ctx) on the same composite re-runs every inner validator (no caching)", () => {
    const calls: string[] = [];
    ...
    // Both inner validators ran twice, once per call, in original order; the composite must
    // produce a fresh array per call, so a hidden result cache would be caught by identity.
  ...
  it("when an inner validator throws, the composite propagates the throw without swallowing or short-circuiting earlier validators", () => {
    // Order: ok -> throws -> never-runs. If the composite swallowed the throw, the third
    // validator would run; if it short-circuited, "first" would be absent from the log.
    const calls: string[] = [];
  ...
  it("mutating the array returned from validate() does not affect a subsequent call's result (defensive separation)", () => {
    // The composite must not retain or reuse the array it returns across calls.
