/**
 * Unit tests for {@link LifecycleHookValidator} hook placement and lifetime signal conflicts.
 */
...
import { LifecycleHookValidator } from "../../../../../src/lib/validation/service/lifecycle-hook-validator.js";
...
/** Returns a ControllerClass stub with optional lifecycle hooks on the prototype. */
...
/** Returns a MiddlewareClass stub with optional lifecycle hooks on the prototype. */
...
describe("lifecycle hook placement by registration kind", () => {
  ...
  it("reports CONTRADICTORY_LIFECYCLE_HOOKS (taking precedence over INVALID_LIFECYCLE_HOOK) when a scoped service has onStart and dispose", () => {
    ...
    // CONTRADICTORY_LIFECYCLE_HOOK takes precedence; no INVALID_LIFECYCLE_HOOK is emitted.
  ...
  it("flags hooks defined on a class higher in the prototype chain (between the user class and the base)", () => {
    ...
    // intermediate subclass in the prototype chain.
    expect(errs[0]!.message).toBe(
