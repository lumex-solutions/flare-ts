/**
 * Pins mockContainer relaxed DI walk: singleton-per-token resolution, missing-dep
 * throws at resolve site, and controller parity with the production container.
 * Drives mockContainer directly and via host.build().test() for cross-arm comparison.
...
import { ControllerBase, FlareResponse, FlareService, Logger } from "../../../../../src/index.js";
...
describe("Edge Cases", () => {
  it("an empty services map produces a container whose every resolveDep throws 'not registered' and never returns undefined", () => {
