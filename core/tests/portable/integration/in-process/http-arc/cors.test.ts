/**
 * Integration tests for HTTP CORS policy behavior: arc-level and per-route
 * policies, preflight OPTIONS handling, allowed/denied origins, and group-level
 * overrides. Each describe builds its own host so policies do not bleed across
 * scenarios. FLARE_MODE must be set before importing FlareHost so the node
  ...
  describe("Actual request from allowed origin (allowlist + credentials + expose)", () => {
    ...
    it(
      ...
      async () => {
        ...
        // responses; Vary applies to the non-wildcard policy class.
        expect(res.headers.get("vary")).toBe("Origin");
...
describe("Edge Cases", () => {
  ...
  describe("Group-level CORS replaces arc-level CORS for routes inside the group", () => {
    ...
    beforeAll(async () => {
      ...
      host.http.cors({ origins: ["https://a.test"] });
      // Outside-group route uses the arc-level policy.
    ...
    it(
      ...
      async () => {
        // b.test is the group's allowlist and should get CORS headers inside the group.
...
describe("Cross-Feature Interactions", () => {
  describe("(with http-arc/pipeline-codegen) async pipeline result + async origin predicate", () => {
    ...
    beforeAll(async () => {
      ...
      // Async handler: the pipeline result is a Promise<ResponseLike>.
