...
const Api = httpContract({ getThing: { route: { name: str } } });

const host = testHost();

// Loose form: a route param + a parsed query are inferred onto scope.input.
...
// Branded form: the descriptor is pulled from a httpContract entry - same inference as loose.
...
// Cross-arc rejection: a socketContract entry is not an httpContract entry - the per-entry brand
// carries the contract's kind, so a "ws" contract entry is rejected in this HTTP context.
const Chat = socketContract({ chat: { query: { room: str } } });
// @ts-expect-error - a "ws" contract entry cannot be used as an HTTP route's `contract`.
host.http.get("/bad-kind", { contract: Chat.chat }, () => null);
