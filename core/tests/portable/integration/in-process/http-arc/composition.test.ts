/**
 * Integration tests for HTTP arc composition registration: controllers,
 * function handlers, middleware, lifecycle hooks, and error handlers. Runs
...
import type { HandlerResult, FlareHost } from "../../../../../src/index.js";
...
/** Records lifecycle hook order across the Primary Behavior suite; each test clears entries before fetching. */
const lifecycleLog: string[] = [];

/** State token shared by middleware `provides` and controller `state` references in Primary Behavior and Cross-Feature suites. */
...
/** Builds the shared Primary Behavior host with controllers, middleware, lifecycle hooks, and error handlers. */
function buildPrimaryHost(): FlareHost<typeof node> {
  ...
  // Multiple method calls on the same path merge into one synthetic controller with two methods.
