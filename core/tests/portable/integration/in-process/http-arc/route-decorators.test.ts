/**
 * Integration tests for HTTP route decorator registration: Get/Post/Method
 * metadata, path composition on controller classes, and decorator-driven dispatch
 * through a fully composed app. Runs in-process via `app.test()` without
 * binding a real port. FLARE_MODE must be set before importing FlareHost so
 * the node adapter's `env: process.env` live binding sees it during host
 * construction.
 */
process.env["FLARE_MODE"] = "test";

import { afterEach, describe, expect, it } from "vitest";
import { Get, Method, Post } from "../../../../../src/decorators.js";
import { ControllerBase, FlareResponse } from "../../../../../src/index.js";
import { testHost } from "../../../helpers/test-host.js";

// Re-arms FLARE_MODE because other tests may mutate process.env.
afterEach(() => {
  process.env["FLARE_MODE"] = "test";
});
    ...
    async () => {
      class WidgetsController extends ControllerBase {
        ...
        public async index() {
          ...
          // slash / trailing-slash checks (path !== "" is false, so no slash
