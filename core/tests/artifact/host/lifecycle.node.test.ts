// FLARE_MODE is set process-wide so the default `node` and `cf` adapters
// (whose `env` is bound to `process.env`) see test mode when constructing a
// FlareHost. Tests that exercise the real (production) FlareAppNode /
// FlareAppCF runtime classes opt OUT by temporarily clearing this flag, then
// restoring it before yielding to the next test.
...
// Shared scaffolding
//
// Every test in this file watches the order in which `onStart` / `onStop`
// fires across the HTTP arc, singleton services, and the framework logger.
// `events` is a per-host log of `<phase>:<name>` strings appended by hook
// implementations inside each helper class below. Tests build their own host
// and their own events array so observations stay isolated.

type LifecycleEvent = string;

/**
 * A silent transport that does not write to console. Used by every host built
 * in this file so test output stays clean. Transports themselves have their
 * own onStart/onStop hooks; we use them when we need a "logger fails to
 * start" or "logger fails to stop" scenario without subclassing Logger
 * itself (Logger is constructed by the adapter, not user-land).
 */
...
describe("Primary Behavior", () => {
  ...
  it(
    ...
    async () => {
      ...
      // On start: HttpArc → Logger (first in singleton walk) → A → B.
      // On stop: B → A (reverse singleton walk excluding Logger) →
      // HttpArc → Logger (last).
      expect(events).toEqual([
        "start:HttpArc",
        "start:Logger",
  ...
  it(
    ...
    async () => {
      ...
      class FailingStartTransport extends LoggerTransport {
        ...
        override async onStop(): Promise<void> {
          // This must NEVER fire — if it does the spec invariant is violated.
...
describe("Failure Modes", () => {
  it(
    ...
    async () => {
      ...
      // `errors` array — required so the runtime can surface diagnostics.
      let caught: unknown;
      try {
        await app.stopAsync();
      } catch (err) {
    ...
    async () => {
      ...
      try {
        const adapter: HostRuntimeAdapter<FlareAppBase> = {
          ...
          defaultLoggerTransports: [SilentTransport],
          createApp(host) {
            // Use the real FlareAppNode by re-importing through the node
            // adapter's createApp — that constructor sets up the signal
            // handlers and the server lifecycle the spec references.
