// FLARE_MODE is set process-wide so any host built with the real `node`
// adapter (which reads `process.env`) would otherwise land in test mode. The
// tests below construct custom adapters with a scrubbed `env` so the FlareHost
// constructor and config-resolution path see only what each test stages.
...
// Bespoke adapter constructed inside each test. Defaults piggy-back on the
// real `node` adapter for `createApp` / `createLogger` / `createTestRequest`
// so we only exercise the host's adapter integration, not a hand-rolled app
// runtime. Each helper overrides exactly the surfaces the test cares about.
...
describe("Failure Modes", () => {
  ...
  it(
    ...
    () => {
      ...
      // Confirm we actually reached createApp — i.e. validation passed and the
