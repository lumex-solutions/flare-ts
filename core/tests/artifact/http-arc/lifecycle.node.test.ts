// FLARE_MODE is set at module load so the default `node` and `cf` adapters
// (whose `env` is bound to `process.env`) see test mode when constructing a
// FlareHost. The custom adapters below also pass FLARE_MODE through their own
// `env` field so they do not depend on process.env at all.
...
// Shared scaffolding
//
// The spec targets `host.http.onStart(fn)` / `host.http.onStop(fn)` — the
// HTTP-arc-level lifecycle hooks. Driving these requires a host whose arc we
// can register callbacks against and an app we can start/stop directly. Each
// test builds its own host so callback arrays and host state never leak
// across cases.
//
// `buildAsyncAdapter()` mirrors `node` but uses an in-memory flare.json and a
// silent transport so test output stays clean and we never touch the real
// filesystem. `buildSyncAdapter()` mirrors `cf` for the same reason and so
// we can drive `app.start()` / `app.stop()` synchronously — the only path
// that exercises `[START_HTTP_ARC]` / `[STOP_HTTP_ARC]` (which reject
...
describe("Primary Behavior", () => {
  ...
  it(
    ...
    async () => {
      ...
      await app.stopAsync();
      // Registration order, NOT reverse — the spec is explicit that
      // http-arc onStop callbacks are walked in registration order.
      // Reverse-order teardown is a singleton-service property documented
      // under host/lifecycle, a separate feature.
      expect(events).toEqual(["first", "second", "third"]);
    },
  );
...
describe("Cross-Feature Interactions", () => {
  it(
    ...
    async () => {
      ...
      await app.startAsync();
      try {
        // The compiled router resolved /ping → handler → 200. Had compilation
