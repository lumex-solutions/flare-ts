...
// Tests that need the LIVE runtimes (which gate behavior on FLARE_MODE) build
// their own adapter with an empty `env`, mirroring the request-id test pattern.
...
// Strip ANSI escapes so pretty-mode strings can be matched against the
// visible content. Mirrors the helper in error-records.test.ts.
function stripAnsi(value: string): string {
  return value.replace(/\x1b\[[0-9;]*m/g, "");
}

// Adapter helper: inject test config and drop the default ConsoleTransport so
// the recording transport is the only sink.
...
// Live-runtime adapter (no test-mode shim). An empty `env` keeps the host out
// of test mode so `host.build()` returns FlareAppNode (which performs the
// per-request loggerALS wrap). Used by tests that assert the Node runtime
...
describe("Failure Modes", () => {
  ...
  it("ALS does NOT flow into a callback invoked outside the original loggerALS.run scope; captureLogStore + runWithLogStore restores context", async () => {
    ...
    // invoke it from the outer scope after the run returns — the same
    // observable shape the docs describe for `waitUntil`.
    const adapter = makeAdapter({
      host: { env: "test" },
      log: { level: "info", enableContext: true },
    });
