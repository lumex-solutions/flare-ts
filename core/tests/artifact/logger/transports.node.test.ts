...
describe("Edge Cases", () => {
  ...
  it("delivers the shutdown ready-trace for transport i only to transports 0..i-1; the just-stopped transport does not see its own shutdown trace", async () => {
    ...
    // also delivered to transport i — that's why we filter on event=ready
    // here, which is the trace the spec describes.)
...
describe("Failure Modes", () => {
  ...
  // The spec bullet "A transport whose onStop throws does not prevent
  // earlier-registered transports from also stopping (best-effort drain)" is
  // deferred. The current `Logger.#stopAsync` implementation does NOT guard
  // individual `transport.onStop()` calls with try/catch, so a throwing onStop
  // aborts the reverse-order loop and earlier-registered transports do not
  // get their onStop invoked. Writing this as a passing test would require
  // either changing the implementation (out of scope for the test writer) or
  // codifying the current "first throw wins, earlier transports skipped"
