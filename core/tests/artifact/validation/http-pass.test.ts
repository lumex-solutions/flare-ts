...
// Behavior tests in validation hit FlareHost.build() directly to exercise the
// pre-build pass; the well-formed-host test then drives it through .test().
process.env["FLARE_MODE"] = "test";

import { describe, expect, it } from "vitest";
import type { CONTRACT_BRAND } from "../../../src/lib/arcs/http/composition/contract/flare-contract.js";
...
// Helpers
//
// Every test builds its own host so a validation failure in one describe
// cannot poison another. The node adapter binds `env: process.env` lazily,
// so FLARE_MODE=test (set above) makes every host land in test mode without
// needing a custom adapter.
...
describe("Edge Cases", () => {
  ...
  it(
    "function routes registered through one HttpBase instance are treated as one synthetic controller — they do not trigger DUPLICATE_ROUTE_PIPELINE against each other",
    () => {
  ...
  it(
    "middleware without any routes fails build — FlareRouter requires at least one route",
    () => {
  ...
  it(
    ...
    () => {
      ...
      } as unknown as { readonly [CONTRACT_BRAND]: true; };
...
describe("Failure Modes", () => {
  it(
    "when several HTTP-level problems exist, every error is collected — the build error includes entries from multiple inner validators",
    () => {
  ...
  it(
    "warning-severity entries (ORPHANED_CONTRACT_ENTRY, DEAD_MIDDLEWARE) do not by themselves fail the build",
    () => {
      // Trigger both warning-only validators:
      //   - contract-alignment: "ghost" key has no matching handler
      //   - dead-middleware:    DeadMw excluded by the only controller via standalone
      const OrphanContract = flareContract({
        list: {},
        ghost: {},
      });
      ...
      class OrphanController extends ControllerBase {
        public static override deps = [];
        public static override state = [];
      ...
      // Push standalone registration directly so DeadMw is excluded by every
      // controller (the only one is standalone) and DEAD_MIDDLEWARE fires.
      host.http.conRegistrations.push({
        factory: (container, ctx) => new OrphanController(container, ctx),
...
describe("Cross-Feature Interactions", () => {
  ...
  it(
    "(with validation/composite) run order is CORS -> RouteSyntax -> RouteParam -> DuplicateRoute -> StateCycle -> Contract -> DeadMiddleware; ordering is observable in the error list",
    () => {
      // Trigger one error per stage (within the http composite). The order in
      // FlareValidationError.errors must mirror the inner-validator order
      // defined by createHttpValidator(). Warning-severity stages (Contract,
      // DeadMiddleware) also ride along in `instance.errors`.
      const StateA = flareState<string>("StateA");
      const StateB = flareState<string>("StateB");
      ...
      const OrphanContract = flareContract({
        list: {},
        ghost: {}, // no handler -> ORPHANED_CONTRACT_ENTRY warning
      });

      class SyntaxBad extends ControllerBase {
        public static override deps = [];
        public static override state = [];
        @Get("/:1bad") // ROUTE_INVALID_PARAM_NAME
        public async one(): Promise<FlareResponse> {
          return new FlareResponse(200, { ok: true });
        }
      }

      class ParamDup extends ControllerBase {
        public static override deps = [];
        public static override state = [];
        @Get("/:id/items/:id") // DUPLICATE_ROUTE_PARAM
        public async two(): Promise<FlareResponse> {
          return new FlareResponse(200, { ok: true });
        }
      }
      ...
      class WithContract extends ControllerBase {
        public static override deps = [];
        public static override state = [];
        public static override contract = OrphanContract;
        @Get("/list")
      ...
      const host = new FlareHost(node);
      host.http.cors({ origins: "*", credentials: true }); // CORS_CREDENTIALS_WILDCARD
      host.http.use(MwA);
      host.http.use(MwB); // MIDDLEWARE_STATE_CYCLE
      // Every controller is registered as standalone (directly into
      // conRegistrations) so DeadMiddleware sees no live controller for
      // MwA/MwB and emits DEAD_MIDDLEWARE warnings for both.
      host.http.conRegistrations.push({
        factory: (container, ctx) => new SyntaxBad(container, ctx),
        cls: SyntaxBad,
        path: "/a",
        standalone: true,
      });
      ...
      // Find the first index of each stage's code in the aggregated list.
      const idx = (code: string) => codes.indexOf(code);
      const idxCors = idx("CORS_CREDENTIALS_WILDCARD");
      const idxSyntax = idx("ROUTE_INVALID_PARAM_NAME");
      const idxParam = idx("DUPLICATE_ROUTE_PARAM");
      const idxDup = idx("DUPLICATE_ROUTE_PIPELINE");
      const idxCycle = idx("MIDDLEWARE_STATE_CYCLE");

      expect(idxCors).toBeGreaterThanOrEqual(0);
      expect(idxSyntax).toBeGreaterThanOrEqual(0);
      expect(idxParam).toBeGreaterThanOrEqual(0);
      expect(idxDup).toBeGreaterThanOrEqual(0);
      expect(idxCycle).toBeGreaterThanOrEqual(0);

      // Order through error-severity stages only; warnings (Contract, DeadMiddleware)
      // are not on the thrown FlareValidationError.errors array.
      expect(idxCors).toBeLessThan(idxSyntax);
      expect(idxSyntax).toBeLessThan(idxParam);
      expect(idxParam).toBeLessThan(idxDup);
      expect(idxDup).toBeLessThan(idxCycle);
    },
  );
  ...
  it(
    "(with validation/error-reporting) error-severity entries surface on FlareValidationError.errors; warnings are logged separately (see warning-only test at line 412)",
    () => {
