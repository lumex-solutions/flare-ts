...
// Behavior tests for the validation/integration spec. The "feature under
// test" is the end-to-end pipeline that composites three layer validators
// (service, http, config) inside FlareHost.build() and translates their
// aggregated results into a single FlareValidationError. Each test builds
// its own host so failures in one scenario cannot poison another.
...
describe("Primary Behavior", () => {
  it(
    ...
    () => {
      ...
      //              structural pattern (/:x vs /:y) — DuplicateRouteValidator
  ...
  it(
    "the order of entries in the thrown error reflects the documented pass order (host.build() invokes service -> http -> config)",
    () => {
...
describe("Edge Cases", () => {
  it(
    "a host with only warning-severity entries builds successfully but exposes the warnings on the path the framework uses (logger transport)",
    async () => {
      // The framework's implemented path for warning exposure is the logger:
      // FlareHost.#build() emits each warning via `this.logger.warn(...)`
      // after compile. Register a capturing transport and a host that trips
      // exactly one warning (DEAD_MIDDLEWARE — the global middleware is
      // excluded by every controller because the only controller is
      // standalone).
      ...
      // Standalone registration excludes ALL global middleware so DeadMw is
      // dead and the dead-middleware validator (warning severity) fires.
      ...
      try {
        ...
        expect(warn!.message).toContain("DeadMw");
      } finally {
        await app.stop();
      }
    },
...
describe("Failure Modes", () => {
  ...
  it(
    ...
    () => {
      ...
      // Programmatic filter — the precise consumer pattern called out in
      // the spec bullet. Stable code means future revisions cannot rename
      // it without breaking this assertion.
...
describe("Cross-Feature Interactions", () => {
  ...
  it(
    ...
    () => {
      ...
      // Classic try/catch shape — what a documented `@throws` consumer writes.
