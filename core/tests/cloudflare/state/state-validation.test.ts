// Build-time validation for DO state crossing (Task 5):
//  - DO-consume seam: a per-DO route consuming a `static state` token validates clean;
//    consuming an unprovided token fails the build.
//  - Front-door provide: each `static state` token of a mounted DO must be provided by a
//    default/derivation, a front-door before-middleware's `provides`, or `resolve.provides`,
//    else MOUNT_STATE_NOT_PROVIDED.
//  - Group/mount overlap hardening: a mount at or under a front-door group prefix is a
//    MOUNT_ROUTE_CONFLICT; group routes are expanded to full paths in the overlap snapshot.
...
describe("front-door provide (MOUNT_STATE_NOT_PROVIDED)", () => {
  ...
  it("an output-only static state token (set by DO, consumed by no DO route) builds clean", () => {
    ...
    const room = host.durableObject(Room);
    // No DO route consumes Out -> no inbound provision required.
