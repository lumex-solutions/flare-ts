 ...
 * the handler's response builder UNTOUCHED. Reconstructing it on the non-FlareResponse branch
 * (`new Response(body, {status, headers})`) drops the `webSocket` client socket and breaks the
 * upgrade; requestIdHeader: true drives that branch.
 *
...
// requestIdHeader: true drives the non-FlareResponse branch that must not reconstruct the Response.
function cfJsonWithReqId(): JsonObject {
  return { host: { env: "test", requestIdHeader: true }, log: { level: "fatal", format: "json" } };
}

/**
 * Stub namespace whose getByName returns a real WebSocketPair 101 response.
