...
// (Bindings, buildCf, cf, DurableState, FlareDurableObject). Each entry is a
// runtime symbol that the subpath barrel MUST expose. FlareDurableObject is
// the public base class users extend when writing Durable Object workers
// (registered via host.durableObject(Class)). The legacy `buildDurableCf` /
// `durableCf` symbols are gone. composeDurableInstance is internal only.

const EXPECTED_RUNTIME_NAMES = ["Bindings", "buildCf", "cf", "durable", "DurableState", "FlareDurableObject", "forwardDurable"] as const;
