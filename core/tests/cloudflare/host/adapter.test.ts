...
// driven through real `host.http.*` routes against the `.export()` terminal.
...
describe("cf adapter (module-scope constant)", () => {
  ...
  it("createApp(host) returns a CloudflareApp bound to host (exposes the export terminal)", () => {
    // CloudflareApp is a type-only export, so assert by terminal shape rather
    // than `instanceof`: createApp must hand back the app exposing the export
    // terminal. Durable Objects are registered via host.durableObject(Class),
    // not an app terminal.
    const host = new FlareHost(cfProdAdapter(cfJson()));
    host.http.get("/_", () => new FlareResponse(200));
    host.build();
    const app = cf.createApp(host);
    expect(typeof app.export).toBe("function");
  });
...
describe("CloudflareApp #handleRequest (exercised via export().fetch)", () => {
...
describe("CloudflareApp #buildResponse (exercised via export().fetch)", () => {
