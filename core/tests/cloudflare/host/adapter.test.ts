...
// live CloudflareApp rather than the test-mode shim. The behavior the old
// FlareAppCF unit tests probed via a stub host + injected `http.fetch` is now
// driven through real `host.http.*` routes against the `.worker()` terminal.
...
import { buildCf, cf } from "../../../src/lib/host/runtime/cloudflare/index.js";
...
describe("cf adapter (module-scope constant)", () => {
  ...
  it("createApp(host) returns a CloudflareApp bound to host (exposes the worker/durableObject terminals)", () => {
    // CloudflareApp is a type-only export, so assert by terminal shape rather
    // than `instanceof`: createApp must hand back the app exposing both terminals.
    const host = new FlareHost(cfProdAdapter(cfJson()));
    host.http.get("/_", () => new FlareResponse(200));
    host.build();
    const app = cf.createApp(host);
    expect(typeof app.worker).toBe("function");
    expect(typeof app.durableObject).toBe("function");
  });
...
describe("CloudflareApp constructor (request-id header + timing config)", () => {
  it("requestIdHeader === true => x-request-id present on response (observed via worker().fetch())", async () => {
...
describe("CloudflareApp.worker()", () => {
  it("invokes start() (drives http arc), sets host state to 'ready', returns { fetch }", () => {
    const host = new FlareHost(cfProdAdapter(cfJson()));
    host.http.get("/x", () => new FlareResponse(200, "ok"));
    const handle = (host.build() as CloudflareApp).worker();

    // worker() walks the http arc via [START_HTTP_ARC] and flips host state to
...
describe("CloudflareApp #handleRequest (exercised via worker().fetch)", () => {
  ...
  it("a throwing route handler never escapes — worker().fetch resolves to 500 with application/json and the Internal Server Error body", async () => {
...
describe("CloudflareApp #buildResponse (exercised via worker().fetch)", () => {
...
describe("CloudflareApp error response (exercised via worker().fetch with a throwing handler)", () => {
...
describe("CloudflareApp #getRequestNonce (memoization observed via x-request-id prefix)", () => {
