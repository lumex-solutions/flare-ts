 ...
 * `DurableObject` base rejects a fake `ctx`, so the generated DO *class* can only be constructed by
 * the runtime — tests drive the real per-instance composition (`composeDurableInstance`) instead.
 */
...
/** Minimal `ExecutionContext` stub for the `.worker()` fetch signature. */
