...
// write/read/alarm glue that the read-only `do-binding.test.ts` smoke cannot, and were previously
// undoable on Windows: the old (pool <= 0.12) per-test isolated-storage teardown unlinked the DO's
// SQLite while workerd still held it open (cloudflare/workers-sdk #10511 / #9913 / #11031). Pool >= 0.15
// removed that filesystem unlink entirely — isolation is now per test FILE, and `reset()` /
// `abortAllDurableObjects()` reset state via `workerd:unsafe` (in-memory, no unlink), so these run
// everywhere. Each test uses a unique DO id, and `afterEach(reset)` wipes all binding data between
// tests (the documented isolation pattern, and a continuous exercise of the Windows-safe reset path).
...
describe("Flare Durable Object storage via a real binding", () => {
  it("persists writes across requests to the same instance (POST /bump → GET /n)", async () => {
  ...
  it("reset() clears persisted DO storage without a filesystem unlink (the Windows-safe path)", async () => {
    ...
    // Storage wiped → the rebuilt instance's `init` finds nothing → counter back to 0.
