...
// ===========================================================================
// 4. LIFETIME - framework singletons (DurableState, Bindings) persist across
//    requests; scoped services are fresh per request
// ===========================================================================
...
describe("durableObject({ init }) runs the init entrypoint", () => {
  it(
    ...
    async () => {
      // Full init/alarm storage round-trips are covered by the real-binding suite in do-storage.test.ts.
      const host = new FlareHost(cfProdAdapter(cfJson()));
      ...
      // init is now a class constructor body; the DI piece it relies on is facade resolution:
...
// ===========================================================================
// 7. alarm entrypoint - the instance's inject(DurableState) is wired correctly
// ===========================================================================
...
describe("durableObject({ alarm }) runs the alarm entrypoint", () => {
  it(
    ...
    async () => {
      // alarm is now a DO class method; the DI piece it relies on is facade resolution
      // via inst.inject(). Full init/alarm storage round-trips are covered by
      // the real-binding suite in do-storage.test.ts.
      const host = new FlareHost(cfProdAdapter(cfJson()));
      host.http.get("/_", () => new FlareResponse(200));
      ...
      // alarm is now a class method; the DI piece it relies on is facade resolution:
      const ds = inst.inject([DurableState], DurableState);
...
describe("per-instance facade: inject + config", () => {
  it("inject() returns the per-instance cached instance for a declared token", () => {
  ...
  it("inject() seeds DurableState and Bindings per instance", () => {
    const host = new FlareHost(cfProdAdapter(cfJson()));
    host.http.get("/_", () => new FlareResponse(200));
  ...
  it("inject() throws when the token is not in the declared deps", () => {
  ...
  it("inject() resolves a registered scoped service from the per-instance container", () => {
...
// ===========================================================================
// NEW: Static FlareDurableObject base + host.durableObject()
// ===========================================================================

describe("FlareDurableObject static base + host.durableObject()", () => {
...
// ===========================================================================
// INJECT KEY RESERVATION - "config" key is reserved on the handler scope
// ===========================================================================
...
// ===========================================================================
// 9. GATING - DurableState on a front-door route is caught at host.build()
// ===========================================================================

describe("revalidation gates DurableState to the durable terminal", () => {
  it("a front-door route injecting DurableState THROWS at host.build() (gate moved off .export())", () => {
    const host = new FlareHost(cfProdAdapter(cfJson()));
    host.http.get(
      "/needs-state",
...
// ===========================================================================
// 10. HANDLER_ERRORED suppresses outbound state (white-box composeDurableInstance)
//
// The B3 real-binding suite (do-state-e2e.test.ts) proves the property end-to-end
// via a real workerd binding. This suite proves the same property using
// composeDurableInstance directly (no workerd), which lets us inspect the raw
// Response returned by inst.fetch() and assert that the x-flare-state header is
// absent when the DO route throws after calling ctx.state.set().
//
// The property: when a handler throws (setting ctx[HANDLER_ERRORED] = true in the
// exec-codegen error dispatch), encodeStateEnvelope is skipped in handler.ts
// #buildResponse, so no outbound state envelope appears on the error response.
// ===========================================================================
  ...
  it(
    ...
    async () => {
      ...
      throwRoom.http.get("/throw-after-set", (ctx) => {
        // Set outbound state before throwing.
      ...
      // Drive the route that sets state then throws.
